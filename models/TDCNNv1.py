...
class TDCNNv1:
    ...
    def generateData(self, train_set, val_set, normalize = True):
        '''
        TODO
        '''
        
        NDVItrain = datareader.generator('NDVI',train_set, 
                                         self.bs,
                                         self.t_len,
                                         self.shape,
                                         normalize = normalize)
                                         
        NDWItrain = datareader.generator('NDWI',train_set, 
                                         self.bs,
                                         self.t_len,
                                         self.shape,
                                         normalize=normalize)
        NDDItrain = datareader.generator('NDDI',train_set, 
                                         self.bs,
                                         self.t_len,
                                         self.shape,
                                         normalize=normalize)
                                         
        NDVIval = datareader.generator('NDVI',val_set,
                                         self.bs,
                                         self.t_len,
                                         self.shape, 
                                         normalize=normalize)
        NDWIval = datareader.generator('NDWI',val_set,
                                         self.bs,
                                         self.t_len,
                                         self.shape, 
                                         normalize=normalize)
        NDDIval = datareader.generator('NDDI',val_set,
                                         self.bs,
                                         self.t_len,
                                         self.shape, 
                                         normalize=normalize)                            
        ...
        self.NDVIdata = (NDVItrain,NDVIval,NDVIval2)
        self.NDWIdata = (NDWItrain,NDWIval,NDWIval2)
        self.NDDIdata = (NDDItrain,NDDIval,NDDIval2)
        
        pass
        
        
    def train(self, indice, train_set, val_set, normalize=True):
        '''
        ...
        if self.NDWIdata is None or self.NDVIdata is None or self.NDWIdata is None: 
            print('ERROR: need to generate data before training model')
            raise
        
        # Training and Validation data loader
        if indice == 'NDWI':
            train_gen = self.NDWIdata[0]
            val_gen = self.NDWIdata[1]
            val_gen_2 = self.NDWIdata[2]
        elif indice == 'NDVI':
            train_gen = self.NDVIdata[0]
            val_gen = self.NDVIdata[1]
            val_gen_2 = self.NDVIdata[2]
        elif indice == 'NDDI':
            train_gen = self.NDDIdata[0]
            val_gen = self.NDDIdata[1]
            val_gen_2 = self.NDDIdata[2]
        else:
            raise
