...
class datareader:
    ...
    def generator(index,dataset, batch_size, t_len, img_shape, normalize=False):
        ...
        '''
        # OPTIMIZE!!
        ndwi_in = np.zeros((batch_size, t_len-1, img_shape[0], img_shape[1], 1))
        ndwi_ou = np.zeros((batch_size, img_shape[0], img_shape[1], 1))
        ndvi_in = np.zeros((batch_size, t_len-1, img_shape[0], img_shape[1], 1))
        ndvi_ou = np.zeros((batch_size, img_shape[0], img_shape[1], 1))
        nddi_in = np.zeros((batch_size, t_len-1, img_shape[0], img_shape[1], 1))
        nddi_ou = np.zeros((batch_size, img_shape[0], img_shape[1], 1))
        ...
        while True:
            ...
            for b in range(batch_size):
                ...
                for i in range(t_len):
                    ...
                    NDVI = spectral_indices.normalized_difference(img, (7,3))
                    # NDWI = spectral_indices.normalized_difference(img, (2,7))
                    NDWI = spectral_indices.normalized_difference(img, (7,11))
                    # NDWI = spectral_indices.normalized_difference(img, (7,3))
                    # NDVI = spectral_indices.normalized_difference(img, (2,7))
                    NDDI = spectral_indices.compute_NDDI(NDWI,NDVI)
                    if i < (t_len - 1):  
                        ndwi_in[b, i, :, :, 0] = NDWI 
                        ndvi_in[b, i, :, :, 0] = NDVI
                        nddi_in[b, i, :, :, 0] = NDDI
            ...
            if index == 'NDWI':
                yield ndwi_in, ndwi_ou
            elif index == 'NDVI':
                yield ndvi_in, ndvi_ou
            elif index == 'NDDI': 
                yield nddi_in, nddi_ou 
            else:
                raise
    ...
    def generatorv2(index, dataset, batch_size, t_len, img_shape, normalize=False):
        ...
        if index == 'NDWI':
            return ndwi_in, ndwi_ou
        elif index == 'NDVI':
            return ndvi_in, ndvi_ou
        elif index == 'NDDI': 
            return nddi_in, nddi_ou 
        else:
            raise
